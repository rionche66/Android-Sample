...
public class Get_Friend_Wishes extends AsyncHttpRequestSample {
	...
	String user_id;		
	Get_Friend_Wish_Result_Request get_wish_result_request;
	
	public Get_Friend_Wishes(Context context,String user_id, Get_Friend_Wish_Result_Request gw) {
		super(context);
            	...
            	try {
					...
					if ( data != null && data.length() >= 0 ) {
						...
						for (int i = 0; i < data.length(); i++) {
							...
							if (json_temp.getString("count_like").equals("null")) {
								temp.set_like_count(0);
							}
							else{
								temp.set_like_count(Integer.parseInt(json_temp.getString("count_like")));
							}					
							
							if (json_temp.getString("is_like").equals("null")) {
								temp.set_is_like(0);
							}
							else{
								temp.set_is_like(1);
	...
	public String getDefaultURL() {
		return HttpConstants.GET_FRIEND_WISH;
	}
